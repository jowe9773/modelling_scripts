import os
import re
import glob
import numpy as np
import pandas as pd
import matplotlib
matplotlib.use('Agg')   # non-interactive backend — no Tk window needed
import matplotlib.pyplot as plt
import matplotlib.patches as patches
import cv2

# ============================================================
# USER SETTINGS
# ============================================================

RASTER_FOLDER = r"C:/Users/josie/OneDrive - UCB-O365/Floodplain LW transport modelling/Playing around with the model/1_0_playing_with_model.gid/Rasters/Hydraulic"
WOOD_FILE     = r"C:/Users/josie/OneDrive - UCB-O365/Floodplain LW transport modelling/Playing around with the model/1_0_playing_with_model.gid/Wood/Wood.rep"
OUTPUT_MOVIE  = r"C:/Users/josie/OneDrive - UCB-O365/Floodplain LW transport modelling/Playing around with the model/wood_flow_animation.mp4"

FPS           = 10
DPI           = 200          # controls output resolution
VARIABLE_NAME = "Depth"
CMAP          = "Blues"
V_MIN         = 0
V_MAX         = 0.06

# ============================================================
# LOAD WOOD DATA
# ============================================================

wood = pd.read_csv(
    WOOD_FILE,
    sep=';',
    comment='#',
    skip_blank_lines=True,
    skipinitialspace=True
)

# Keep only fully numeric rows
wood = wood[pd.to_numeric(wood["Time(s)"], errors='coerce').notnull()]

for col in wood.columns:
    wood[col] = pd.to_numeric(wood[col])

wood.columns = [c.strip() for c in wood.columns]
wood["Time(s)"] = wood["Time(s)"].astype(float)

print(wood)

# ============================================================
# FIND ALL RASTERS
# ============================================================

raster_files = sorted(
    glob.glob(os.path.join(RASTER_FOLDER, f"{VARIABLE_NAME}*.asc"))
)

# ============================================================
# READ ASCII RASTER
# ============================================================

def read_ascii_raster(filepath):
    with open(filepath, 'r') as f:

        ncols    = int(f.readline().split()[1])
        nrows    = int(f.readline().split()[1])

        # Header can be XLLCENTER or XLLCORNER — same for YLL
        xll_line = f.readline().split()
        yll_line = f.readline().split()
        xll_type = xll_line[0].upper()
        yll_type = yll_line[0].upper()
        xll      = float(xll_line[1])
        yll      = float(yll_line[1])

        cellsize = float(f.readline().split()[1])
        nodata   = float(f.readline().split()[1])
        data     = np.loadtxt(f)

    data[data == nodata] = np.nan

    # CORNER coords refer to the cell edge; shift by half a cell to get cell centres
    if "CORNER" in xll_type:
        xll += cellsize / 2
    if "CORNER" in yll_type:
        yll += cellsize / 2

    # ASC files store row 0 at the top (north); flip so row 0 is at the bottom
    # to match imshow origin='lower' — xll/yll remain the SW corner
    data = np.flipud(data)

    return data, ncols, nrows, xll, yll, cellsize

# ============================================================
# EXTRACT TIME FROM FILENAME  e.g. Depth____30.0.asc
# ============================================================

def extract_time(filename):
    name  = filename.replace(".asc", "")
    parts = name.split("_")
    for part in reversed(parts):
        try:
            return float(part)
        except ValueError:
            continue
    return None

# ============================================================
# LOAD ALL RASTERS
# ============================================================

rasters = []
for file in raster_files:
    time = extract_time(os.path.basename(file))
    data, ncols, nrows, xll, yll, cellsize = read_ascii_raster(file)
    rasters.append({"time": time, "data": data})

rasters = sorted(rasters, key=lambda x: x["time"])

# ============================================================
# DIAGNOSTICS — run once to check coordinate alignment
# ============================================================

print("\n--- RASTER EXTENT ---")
print(f"  XLLCENTER : {xll}")
print(f"  YLLCENTER : {yll}")
print(f"  cellsize  : {cellsize}")
print(f"  ncols     : {ncols},  nrows: {nrows}")
xmin_edge = xll - cellsize / 2
xmax_edge = xll + (ncols - 0.5) * cellsize
ymin_edge = yll - cellsize / 2
ymax_edge = yll + (nrows - 0.5) * cellsize
print(f"  x extent  : {xmin_edge:.4f}  →  {xmax_edge:.4f}")
print(f"  y extent  : {ymin_edge:.4f}  →  {ymax_edge:.4f}")

print("\n--- WOOD COORDINATE RANGE ---")
print(f"  X : {wood['X'].min():.4f}  →  {wood['X'].max():.4f}")
print(f"  Y : {wood['Y'].min():.4f}  →  {wood['Y'].max():.4f}")
print()

# ============================================================
# SETUP FIGURE  (created once, reused every frame)
# ============================================================

fig, ax = plt.subplots(figsize=(14, 6))

# xll/yll are cell centres (after any CORNER→CENTRE correction above)
# imshow extent wants the outer edges of the border cells
xmin   = xll - cellsize / 2
ymin   = yll - cellsize / 2
xmax   = xll + (ncols - 0.5) * cellsize
ymax   = yll + (nrows - 0.5) * cellsize
extent = [xmin, xmax, ymin, ymax]

img = ax.imshow(
    rasters[0]["data"],
    extent=extent,
    origin='lower',
    cmap=CMAP,
    vmin=V_MIN,
    vmax=V_MAX
)

cbar = plt.colorbar(img, ax=ax)
cbar.set_label("Flow Depth (m)")
title = ax.set_title("")
ax.set_aspect('equal')

# ============================================================
# DRAW WOOD PIECES
# ============================================================

wood_patches = []

def draw_wood(frame_time):
    global wood_patches
    for p in wood_patches:
        p.remove()
    wood_patches = []

    tol     = 0.01
    current = wood[np.abs(wood["Time(s)"] - frame_time) < tol]

    for _, row in current.iterrows():
        x         = row["X"]
        y         = row["Y"]
        length    = row["Length(m)"]
        diameter  = row["Diameter(m)"]
        angle_deg = np.degrees(row["Angle"])

        # Place rectangle centred on (x, y) with zero rotation,
        # then rotate about (x, y) via the transform — only one rotation applied
        rect = patches.Rectangle(
            (x - length / 2, y - diameter / 2),
            length,
            diameter,
            angle=0,                  # no built-in rotation
            linewidth=1,
            edgecolor='black',
            facecolor='saddlebrown',
            zorder=10
        )

        t = (
            patches.transforms.Affine2D()
            .rotate_deg_around(x, y, angle_deg)
            + ax.transData
        )
        rect.set_transform(t)
        ax.add_patch(rect)
        wood_patches.append(rect)

# ============================================================
# RENDER ONE FRAME → numpy array (H, W, 3)  BGR for cv2
# ============================================================

def render_frame(raster):
    img.set_array(raster["data"])
    title.set_text(f"Time = {raster['time']:.2f} s")
    draw_wood(raster["time"])

    fig.canvas.draw()

    # buffer_rgba() returns RGBA bytes; works across all modern matplotlib versions
    buf = np.frombuffer(fig.canvas.buffer_rgba(), dtype=np.uint8)
    w, h = fig.canvas.get_width_height()
    frame_rgba = buf.reshape(h, w, 4)

    # cv2 expects BGR (drop the alpha channel)
    return cv2.cvtColor(frame_rgba, cv2.COLOR_RGBA2BGR)

# ============================================================
# INITIALISE cv2.VideoWriter
# ============================================================

# Render frame 0 first so we know the exact pixel dimensions
fig.canvas.draw()          # make sure the canvas is sized at DPI
plt.tight_layout()
fig.set_dpi(DPI)
fig.canvas.draw()

first_frame = render_frame(rasters[0])
frame_h, frame_w = first_frame.shape[:2]

fourcc = cv2.VideoWriter_fourcc(*'mp4v')   # H.264-compatible container
writer = cv2.VideoWriter(OUTPUT_MOVIE, fourcc, FPS, (frame_w, frame_h))

# Write the first frame we already rendered
writer.write(first_frame)
print(f"Frame 1 / {len(rasters)}")

# ============================================================
# RENDER REMAINING FRAMES
# ============================================================

for i, raster in enumerate(rasters[1:], start=2):
    frame = render_frame(raster)
    writer.write(frame)
    print(f"Frame {i} / {len(rasters)}")

# ============================================================
# FINALISE
# ============================================================

writer.release()
plt.close(fig)

print("Animation saved:", OUTPUT_MOVIE)
